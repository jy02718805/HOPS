...
<div class="line_bar">	
	...
	<input id='reportTypeName' type='hidden' name='reportTypeName' value="fundchange"/>
...
订单时间: <select id='changeDate' name='changeDate' onChange='changeSetDate(this);' class="select w80">
	...
	状态:<select name="orderStatus" id="orderStatus" class="select w80">
			<c:choose>
	...
	通知状态:<select name="notifyStatus" id="notifyStatus" class="select w80">
		...
		电话号码:<input id="usercode" name="usercode" type="text"  maxlength="11" value="${order.usercode}" class="ipt w100" onkeyup="value=value.replace(/[^\d]/g,'')"/>
			系统订单号 :<input id="orderNo" type="text" value="${order.orderNo}" class="ipt w100" onkeyup="value=value.replace(/[^\d]/g,'')"/>
			商户订单号 :<input id="merchantOrderNo" type="text" value="${order.merchantOrderNo}" class="ipt w100" />
			
			<div style="float:right;">
			<input type="button" class="button" value="查询" onclick="queryOrder()"/>
			<input type="button" class="button" value="导出" onclick="exportOrder()"/>
			</div>
			<br/>
			<span><input type='checkbox' id='statisticsOrder' name='statisticsOrder' value='1' <c:if test='${order.statisticsOrder==1}'>checked="checked"</c:if>>统计信息</input>: 
				(总面值: <span class='msg1' style="color: red;" > </span>		成功面值: <span class='msg2' style="color: red;"> </span>		待充面值: <span class='msg3' style="color: red;"> </span>  失败面值:<span class='msg4' style="color: red;"></span> 
				 成本金额:<span class='msg5' style="color: red;"> </span> )</span>
...
<!-- 		下单时间	 充值订单号	  代理商   供货商	 运营商	 省份	 充值账号	 金额	 成功	 待充	 订单状态	 通知状态  预成功状态	 完成时间	 操作 -->
		<tr>
			...
			<th><a class=sort href="javascript:;" name=orderFee>面额(元)</a> </th>
			<th><a class=sort href="javascript:;" name=orderSuccessFee>成功面值</a> </th>
			<th><a class=sort href="javascript:;" name=orderWaitFee>待充面值</a> </th>
			<th><a class=sort href="javascript:;" name=orderStatus>订单状态</a> </th>
			<th><a class=sort href="javascript:;" name=notifyStatus>通知状态</a> </th>
