...
<body>
<form id="listForm" method="get" action="deliveryRecordList" onsubmit="return check();">
    ...
    <option value='1' <#if businessType=="1">selected='selected'</#if>>流量</option>
				    ...
				    供货商：<select name = "supplymerchant" id = "supplymerchant" class="select w100">
							...
							<input type="text" id = "numbervalue" name="numbervalue" value="${(numbervalue)!""}"  class="ipt"/>	 
							 <input type="button" onclick='queryDeliverys();' class="button" value="查询" />	  
							 <br />
			<span> 
			<input type='checkbox' id='statisticsOrder' name='statisticsOrder' value='0' <#if statisticsOrder=='1'>checked="checked"</#if>>统计信息:</input>
					(总面值: <span class='msg1' style="color: red;"> </span>		成功面值: <span class='msg2' style="color: red;"> </span>		销售金额: <span class='msg4' style="color: red;"> </span>		成本金额: <span class='msg3' style="color: red;"> </span>)</span>
  ...
  <script type="text/javascript" language="javascript"> 
		...
		function statisticsOrderInfo(){
			...
			numbertype_param = $("#numbertype_agent").val();
				...
				$.ajax({
						url:"${base}/transaction/statisticsDeliveryInfo?carrierInfo="
						+$("#carrier_no").val()
