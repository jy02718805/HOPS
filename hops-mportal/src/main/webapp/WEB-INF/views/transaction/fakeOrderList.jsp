...
<body>
<form id="listForm" method="get" action="${ctx}/transaction/fakeOrderList">
	...
	开始时间:<input id="beginDate" name="beginDate" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-{%M-3}-%d',maxDate:'#F{$dp.$D(\'endDate\')|| \'%y-%M-%d\'}'});changeDateSelect();"  value="${order.beginDate}" class="ipt w150"/>
	结束时间:<input id="endDate" name="endDate" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'beginDate\') || \'%y-{%M-3}-%d\'}',maxDate:'%y-%M-%d'});changeDateSelect();" value="${order.endDate}" class="ipt w150"/>
	<br/>
	...
	订单状态:<select name="status" id="status" class="select w80" >
			<c:choose>
	...
	省份:<select name="province" id="province" class="select w80">
		 ...
		 <br/>
		 代理商商户:<select name="downMerchant" id="downMerchant" class="select w100">
			<option value="">请选择</option>
			<c:forEach items="${merchants}" var="merchant">
				<c:choose>
					<c:when test="${order.downMerchant == merchant.id}">
	...
	<div class="pages_menber">(共<SPAN id=pageTotal>${counttotal}</SPAN>条记录)</div>
 	...
 	<script type="text/javascript" language="javascript">
		...
		function queryOrder(){
			...
			window.location.href ="${ctx}/transaction/fakeOrderList?carrierInfo="
					+$("#carrierInfo").val()
