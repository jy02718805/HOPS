...
<%-- 			</c:choose> --%>
	...
	<!--预成功状态:<select name="preSuccessStatus" id="preSuccessStatus" class="select w80">
		 ...
		 手工处理标识:<select name="manualFlag" id="manualFlag" class="select w80">
         <c:choose>
             <c:when test="${order.manualFlag==''}">
                 <option value="" selected="selected">请选择</option>
             </c:when>
             <c:otherwise>
                 <option value="">请选择</option>
         ...
         <c:choose>
             <c:when test="${order.manualFlag=='0'}">
                 <option value="0" selected="selected" >无需人工审核</option>
             </c:when>
             <c:otherwise>
                 <option value="0">无需人工审核</option>
             </c:otherwise>
         </c:choose>
         <c:choose>
             <c:when test="${order.manualFlag=='1'}">
                 <option value="1" selected="selected" >已转人工审核</option>
             </c:when>
             <c:otherwise>
                 <option value="1">已转人工审核</option>
             </c:otherwise>
         </c:choose>
         <c:choose>
             <c:when test="${order.manualFlag=='2'}">
                 <option value="2" selected="selected" >手工处理完成</option>
             </c:when>
             <c:otherwise>
                 <option value="2">手工处理完成</option>
             </c:otherwise>
         </c:choose>
      </select>
      代理商商户:<select name="merchantId" id="merchantId" class="select w100">
      <option value="">请选择</option>
  ...
  <!-- 供货商商户:--> <select name="supplyMerchant" id="supplyMerchant" class="select w100" style="display:none">
   <option value="">请选择</option>
   <c:forEach items="${supplyMerchants}" var="merchant">
       <c:choose>
         ...
         <c:forEach items="${provinces}" var="province">
		 	...
		 	面值:<input type="text" id="parValue" name="parValue"  maxlength="5" value="${order.parValue}" class="ipt w80"/>
			...
			系统订单号 :<input id="orderNo" name="orderNo" type="text" value="${order.orderNo}" class="ipt w80" onkeyup="value=value.replace(/[^\d]/g,'')"/>
			...
			<c:when test="${fn:length(mlist) > 0}">
				<c:forEach items="${mlist}" var="order" varStatus="status">
					<tr>
						...
						<td>
						<shiro:hasPermission name="order:orderdetail_view">
							<a href="${ctx}/transaction/showOrderDetail?orderNo=${order.ORDERNO}">${order.ORDERNO}</a>
						</shiro:hasPermission>
						<shiro:lacksPermission name="order:orderdetail_view">	
							${order.ORDERNO}
			       ...
			       <c:otherwise>
						...
						<td>${order.ORDERFINISHTIME}
						</td>
						<td>
							<c:choose>
								   <c:when test="${order.ORDERSTATUS==3 && order.BUSINESSTYPE==0}">
								   		<shiro:hasPermission name="order:refundOrder_show">
											<a href="javascript:void(0);" onclick="closeOrder(${order.ORDERNO})">[退款]</a>
										</shiro:hasPermission>
										<!-- shiro:hasPermission name="order:refundOrder_show"-->
											<a href="javascript:void(0);" onclick="reOpenOrder(${order.ORDERNO})">[换号补充]</a>
										<!-- /shiro:hasPermission-->
							       </c:when>
							       <c:when test="${order.ORDERSTATUS!=3 && order.ORDERSTATUS!=4 && order.PRESUCCESSSTATUS==2}">
							       <shiro:hasPermission name="manualAuditOrder:turn_order_show">
                                       <a href="javascript:void(0);" onclick="turnOrder(${order.ORDERNO},0,9)">[转普通订单]</a>
                                   </shiro:hasPermission>
                                   </c:when>
                                   
                                   <c:when test="${order.ORDERREASON==9}">
                                   <shiro:hasPermission name="manualAuditOrder:turn_order_show">
                                       <a href="javascript:void(0);" onclick="turnOrder(${order.ORDERNO},2,10)">[转预成功订单]</a>
                                   </shiro:hasPermission>
                                   </c:when>
							       <c:otherwise>
                            ...
                            <shiro:hasPermission name="manualAuditOrder:reNotity">
                            <a href="javascript:void(0);" onclick="reNotity(${order.ORDERNO})">[重新通知]</a>
                            </shiro:hasPermission>
                            </c:when>
                            <c:otherwise>
...
<div class="line_pages">
	...
	<div class="pages_menber">(共<SPAN id=pageTotal>${counttotal}</SPAN>条记录)</div>
 	...
 	<script type="text/javascript" language="javascript">
		...
		function orderReTry(orderNo){
			window.location.href="${ctx}/transaction/orderReTry?orderNo="+orderNo;
		}
		
		function orderReBind(orderNo){
			window.location.href="${ctx}/transaction/orderReBind?orderNo="+orderNo;
		}
		...
		function queryOrder(){
			...
			window.location.href ="${ctx}/transaction/orderList?carrierInfo="
					+$("#carrierInfo").val()
		...
		{
			var html = $.ajax({ url: "${ctx}/transaction/checkOrder?orderNo="+orderNo+"&url=orderList", async: false }).responseText;
            var title = "订单换号补充";
            art.dialog({
                content: html,
                title: title,
		...
		function turnOrder(orderNo,preSuccessStatus,orderReason)
        {
		    var msg="";
		    if(preSuccessStatus==0){
		        msg="确认转为普通订单?";
		    }else{
		        msg="确认转为预成功订单?";
		    }
		    ...
		    if(confirm(msg)){
		        $.ajax({
	                 ...
	                 type: 'GET',
	                 dataType:'text',
	                 data: "orderNo="+orderNo+"&preSuccessStatus="+preSuccessStatus+"&orderReason="+orderReason, 
