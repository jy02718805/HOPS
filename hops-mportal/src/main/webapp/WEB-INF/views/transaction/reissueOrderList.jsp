...
<body>
	<form id="listForm" method="get"
		action="${ctx}/transaction/reissueOrderList">
		...
		省份:<select name="province" id="province" class="select w80">
			...
			<input type="button" class="button" value="查询" onclick="queryOrder()" />
				...
				<input type="button" class="button" onclick="manualAuditorders();"  value="补发查询" />
				<br/>
		</div>
  	...
  	<TH><input type='checkbox' id='checkorder' name='checkorder' onclick='checkOrders(this);'/></TH>
				...
				<c:choose>
					<c:when test="${fn:length(mlist) > 0}">
						<c:forEach items="${mlist}" var="order" varStatus="status">
							<tr> 
		  <td><input type='checkbox' name='manualAuditorder' value='${order.DELIVERYID}' onclick='unCheckOrders(this);'/></td>
		...
		function queryOrder(){
			...
			window.location.href="${ctx}/transaction/reissueOrderList?&carrierInfo="
				+$("#carrierInfo").val()
		...
		{
			var html = $.ajax({ url: "${ctx}/transaction/checkOrder?orderNo="+orderNo+"&url=reissueOrderList", async: false }).responseText;
            var title = "人工审核操作";
            art.dialog({
                content: html,
                title: title,
		...
		function checkOrders(allCheckbox){
			...
			function manualAuditorders(){
			    ...
			    if(result!=""&&result!=null){
			    	...
			    	window.location.href="${ctx}/transaction/checkReissueDelivery?deliveryId="+result;
// 					$.ajax({
// 				        type: "post",
// 				        data: null,
...
// 			    self.location.href="${ctx}/transaction/reissueOrderList";
// 			    return false;
			}
