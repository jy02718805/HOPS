...
function getMerchantType(mtype){
	...
	document.getElementById("reMerchantId").options.add(new Option('--请选择--','0'));
	if(mtype=="SUPPLY")
	{
		...
		<#list downMerchant as list>						
				document.getElementById("reMerchantId").options.add(new Option('${list.merchantName}','${list.id}')); 
		</#list>
	}else if(mtype=="AGENT")
	{
		<#list downMerchant as list>						
	...
	<form id="inputForm" action=""  method="get">
		<table class="input">
			<tr>
				...
				<td>
					<select name="businessNo" id="businessNo"  onchange="getProductByMerchant('')" class="select">
			...
			<tr>
				...
				<td>
					<select name="reMerchantId" id="reMerchantId"  onchange="getProductByMerchant(this.value)" class="select">
						<option value="" selected=selected>--请选择--</option>
					</select>	
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="carrierId" id="carrierId" onchange="getProductByMerchant(this.value)" class="select">
					<option value="" selected=selected>-请选择-</option>
			...
			<tr>
				...
				<td>
					<select name="provinceId" id="provinceId" onchange="getCtiy(this.value,'2')" class="select">
					<option value="" selected=selected>-请选择-</option>
			...
			<tr>
				...
				<td>
					<select name="cityId" id="cityId" onchange="getProductByMerchant(this.value)" class="select">
						<option value="" selected=selected>--请选择--</option>
					</select>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<input type="button" class="button" value="确&nbsp;&nbsp;定" onclick="addAssignExcludeProduct()"/>
