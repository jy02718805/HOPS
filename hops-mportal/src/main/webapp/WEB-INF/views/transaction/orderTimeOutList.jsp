...
<body>
<form id="listForm" method="get" action="${ctx}/transaction/orderTimeOutList">
						...
						<td>
							<c:choose>
								   <c:when test="${order.BUSINESSTYPE==0}">
										...
										<c:if test="${order.MANUALFLAG != 2}">
											<shiro:hasPermission name="orderouttime:force_fail_order">
												<a href="javascript:void(0);"
													onclick="checkForceFailOrder(${order.ORDERNO})">[强制审核失败]</a>
											</shiro:hasPermission>
										</c:if>
										</c:when>
							       <c:otherwise>
	...
	<div class="pages_menber">(共<SPAN id=pageTotal>${counttotal}</SPAN>条记录)</div>
 	...
 	<script type="text/javascript" language="javascript">
		...
		function queryOrder(){
			...
			window.location.href="${ctx}/transaction/orderTimeOutList?carrierInfo="
				+$("#carrierInfo").val()
