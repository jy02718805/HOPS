...
<script type="text/javascript"
	src="${ctx}/template/common/js/DatePicker/calendar.js"></script>
<script type="text/javascript"
...
<body>
	<form id="listForm" method="get"
		action="${ctx}/transaction/manualAuditOrderList">
		...
		开始时间:<input id="beginDate" name="beginDate" type="text"
				...
				value="${order.beginDate}" class="ipt w150" /> 
		结束时间:<input
				...
				value="${order.endDate}" class="ipt w150" /> 
			<br/>
		    ...
		    <option value='2' <c:if test='${order.businessType=="2"}'>selected='selected'</c:if>>固话</option>
				...
				订单状态:
				<select name="orderStatus" id="orderStatus" class="select w80">
					<c:choose>
			...
			通知状态:
				<select name="notifyStatus" id="notifyStatus" class="select w80">
					<c:choose>
			...
			预成功状态:
				<select name="preSuccessStatus" id="preSuccessStatus"
				class="select w80">
					<c:choose>
			...
			绑定类型: <select id='reBindType' name='reBindType'  class="select w80"  >
		    <option value='' <c:if test='${order.reBindType==""}'>selected='selected'</c:if>>请选择</option>
		    <option value='0' <c:if test='${order.reBindType=="0"}'>selected='selected'</c:if>>待重绑</option>
		    </select>
		...
		省份:<select name="province" id="province" class="select w80">
			...
			<br/>
			代理商商户:
				<select name="agentMerchant" id="agentMerchant" class="select w80">
					<option value="">请选择</option>
			...
			面值:
				<input type="text" id="parValue" name="parValue" size="5"
				value="${order.parValue}" class="ipt w80" /> 
			...
			<input type="button" class="button" value="查询" onclick="queryOrder()" />
				...
				<shiro:hasPermission name="manualAuditorder:auditor_order_list">
					<input type="button" class="button" onclick="manualAuditorders();"  value="批量审核" />
					<select name="supplyMerchants" id="supplyMerchants" class="select w80">
						<option value="">请选择</option>
  	...
  	<TH><input type='checkbox' id='checkorder' name='checkorder' onclick='checkOrders(this);'/></TH>
				...
				<c:choose>
					<c:when test="${fn:length(mlist) > 0}">
						<c:forEach items="${mlist}" var="order" varStatus="status">
							<tr>
		  <td><input type='checkbox' name='manualAuditorder' value='${order.orderNo}' onclick='unCheckOrders(this);'/></td>
								<td>${(page-1)*pageSize+status.index+1}</td>
								<td><fmt:formatDate value="${order.orderRequestTime}"
			       ...
			       <c:otherwise>
							...
							<c:choose>
								...
								<td><fmt:formatDate value="${order.orderFinishTime}"
										type="both" dateStyle="medium" /></td>
								<td>
									<c:choose>
										   <c:when test="${order.businessType==0}">
												...
												<shiro:hasPermission name="manualAuditOrder:turn_order_show">
									   			<a href="javascript:void(0);" onclick="turnOrder(${order.orderNo})">[转普通订单]</a>
									   			</shiro:hasPermission>
									       </c:when>
									       <c:otherwise>
		...
		function toCloseOrder(orderNo){
			var carrierInfo_select = $("#carrierInfo_select").val();
			var provinces_select = $("#provinces_select").val();
			var parValue = $("#parValue").val();
			var merchants_select = $("#merchants_select").val();
			var status_select = $("#status_select").val();
			$("#carrierInfo").val(carrierInfo_select);
			$("#province").val(provinces_select);
			$("#parValue").val(parValue);
			$("#agentMerchant").val(merchants_select);
			$("#status").val(status_select);
			window.location.href="${ctx}/transaction/toCloseManualAuditOrder?orderNo="+orderNo+"&time=5&timeUnit=m&carrierInfo="+$("#carrierInfo").val()+"&province="+$("#province").val()+"&parValue="+$("#parValue").val()+"&downMerchant="+$("#agentMerchant").val();
			
		}
		
		function toSuccessOrder(orderNo){
			var carrierInfo_select = $("#carrierInfo_select").val();
			var provinces_select = $("#provinces_select").val();
			var parValue = $("#parValue").val();
			var merchants_select = $("#merchants_select").val();
			...
			$("#agentMerchant").val(merchants_select);
			$("#status").val(status_select);
			window.location.href="${ctx}/transaction/toSuccessManualAuditOrder?orderNo="+orderNo+"&time=5&timeUnit=m&carrierInfo="+$("#carrierInfo").val()+"&province="+$("#province").val()+"&parValue="+$("#parValue").val()+"&downMerchant="+$("#downMerchant").val();
		...
		{
			var html = $.ajax({ url: "${ctx}/transaction/checkOrder?orderNo="+orderNo+"&url=manualAuditOrderList", async: false }).responseText;
            var title = "人工审核操作";
            art.dialog({
                content: html,
                title: title,
		...
		function checkOrders(allCheckbox){
			...
			function manualAuditorders(){
			    ...
			    if(result!=""&&result!=null){
			    	if (!confirm("确认要批量审核这些数据吗?【共"+k+"条】")) {
				        return false;
				    }
			    	window.location.href="${ctx}/transaction/checkOrderList?orderNos="+result+"&supplyMerchantId="+supplyMerchantId;
// 					$.ajax({
// 				        type: "post",
// 				        data: null,
// 				        url: "checkOrderList?orderNos="+result+"&supplyMerchants="+supplyMerchants,
// 						async: false,
// 				        success: function (data) {
// 				        	var re=data.split('*');
// 			                alert(re[1]); 
...
// 			    self.location.href="${ctx}/transaction/manualAuditOrderList";
// 			    return false;
			}
			
			
			
			function turnOrder(orderNo)
			{
				 $.ajax({
					 url:'turnOrder',
					 type: 'GET',
