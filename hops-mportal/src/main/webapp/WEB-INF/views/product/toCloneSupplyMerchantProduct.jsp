...
<script type="text/javascript" src="${ctx}/template/common/js/ztree/jquery.popupSmallMenu.js"></script>
	<title>商户产品批量设置</title>
	
	<script>
	function cloneUpMerchantProduct(){
	...
	//getAllMerchants(obj.id);//1.得到全集
	function getAllMerchants(id){
		$.ajax({
			url:"${ctx}/product/getAllMerchants?merchantType=SUPPLY", 
			type: "post",
	        data: null,
	        async: false,
			success:function(data) {
				$('#product_ids').append(data);
				var merchant = data.split("|");
				for(var i = 0; i < merchant.length; i++ ){
					var merchantPropertys = merchant[i].split(",");
					for(var j = 0; j < merchantPropertys.length; j++ ){
						var merchantId = merchantPropertys[0];
						var merchantName = merchantPropertys[1];
						$("#"+id).empty();
						var option = $("<option>").val(merchantId).text(merchantName);
						$("#"+id).append(option);
					}
				}
			}
	...
	function getUpMerchantProductList(){
		$('#product_ids').html("");
		var oldMerchantId=$('#oldMerchantId').val();
		var newMerchantId=$('#newMerchantId').val();
		var isCommonUse='';
	    ...
	    if(oldMerchantId != -1){
			$.ajax({
				url:"${ctx}/product/getSupplyMerchantProductList?merchantId="+oldMerchantId+"&newMerchantId="+newMerchantId+"&carrierInfo="+carrierInfo+"&businessType="+businessType+"&province="+province
...
<body>
<div class="mg10"></div>
	<form id="inputForm" action="${ctx}/product/cloneSupplyMerchantProduct" method="post" class="form-horizontal">
		<input type="hidden" id="productIds" name="productIds" class="ipt"/>
		<table id="merchant" class="input">
			<tr>
				<th>
					<span class="requiredField">*</span>原供货商:
				</th>
				<td>
		    ...
		    <tr>
				<th>
					<span class="requiredField">*</span>待复制供货商:
				</th>
				<td>
		...
		<table class="input">
			...
			&nbsp&nbsp	省份:<select name="province" id="province" onchange="getCityByProvince(this)" class="select w80">
				<option value="">请选择</option>
			...
			&nbsp&nbsp城市:	<select name="city" id="city" class="select w80">
				...
				&nbsp&nbsp面值:  <input type='text' id='faceValue' name='faceValue' class="ipt w80" value='' onkeyup="value=value.replace(/[^\d]/g,'')"/>
				&nbsp 至 &nbsp
				 <input type='text' id='faceValue2' name='faceValue2' class="ipt w80" value='' onkeyup="value=value.replace(/[^\d]/g,'')"/>
				
					 <input type='button' class="button" value='查询' onclick="getUpMerchantProductList();" />
				<br/>
				&nbsp&nbsp
				 <input type="checkbox" name="to_all" id="to_all"> 全选</input> &nbsp&nbsp
		...
		<table class="input">
				...
				<td>
					<input id="submit_btn" class="button" type="button" onclick="cloneUpMerchantProduct()" value="提交"/>&nbsp;	
