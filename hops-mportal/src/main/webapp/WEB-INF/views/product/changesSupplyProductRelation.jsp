...
<body>
<form id="inputForm" action="${ctx}/product/changesSupplyProductRelation">
<input id='page' type='hidden' name='page'/> 
<input id='pageSize' type='hidden' name='pageSize' value='${pageSize}'/> 
<div class="line_bar">
	供货商商户:<select name="identityId" id="identityId" class='select w120'>
	...
	面     值:<input type="text" id="parValue" name="parValue" value="${productRelation.parValue}"  class='ipt w80'/>
...
<div style="float:right;">
	...
	<shiro:hasPermission name="supplyProduct:editlist_execute">
	<input type="button" class="button"  id="changes_discount" value="批量修改折扣" />
	</shiro:hasPermission>
	<shiro:hasPermission name="supplyProduct:opentlist_execute">
	<input type="button" class="button"  id="changes_open" value="批量开启" />
	</shiro:hasPermission>
	<shiro:hasPermission name="supplyProduct:closelist_execute">
	<input type="button" class="button"  id="changes_close" value="批量关闭" />
	</shiro:hasPermission>
	
	<shiro:hasPermission name="supplyProduct:deletelist_execute">
	...
	<table id=listTable class=list>
		...
		<tr>
		<th>&nbsp;<input type="checkbox" id="checkproduct" name="checkproduct" ></input> </th>
			<th><SPAN>序号</SPAN> </th>
		...
		<c:choose>
			...
			<tr>
			<td><input type="checkbox" name="checkproduct_${status.count}" onclick="clickbox(this);"></input></td>
				<td>${(page-1)*pageSize+status.index+1}</td>
				<td><input type="hidden"  id="id_${status.count}" name="id"  value="${UpProductRelation.id}"/>
				<input type="hidden"  id="productStatus_${status.count}" name="productStatus"  value="${UpProductRelation.airtimeProduct.productStatus}"/>
				${UpProductRelation.identityName}</td>
				<td>
				<input type="hidden"  id="productId_${status.count}" name="productId"  value="${UpProductRelation.productId}"/>
				
				<input type="hidden"  id="businessType_${status.count}" name="businessType"  value="${UpProductRelation.businessType}"/>
				${UpProductRelation.productName}</td>
				<td>
				<input type="text"  size="5" disabled="true" id="discount_${status.count}" onKeyUp="this.value=this.value.replace(/[^\d.]/g,'')" name="discount_${status.count}"  value="${UpProductRelation.discount}"/>
				<input type="button"   id="discountButton_${status.count}" name="discountButton_${status.count}" onclick="discount(${status.count});"  value="同上" style="display:none;" />
				<input type="button" id="discountChangesButton_${status.count}" name="discountChangesButton_${status.count}" onclick="discountChanges(${status.count});"  value="批量同此"  style="display:none;"/>
				</td>
				...
				<td>
				<select id="merchantselect_${status.count}" disabled="true">
					<c:forEach items="${merchantLevels}" var="merchantLevel">
						<option value="${merchantLevel.merchantLevel}"
						<c:if test="${UpProductRelation.merchantLevel==merchantLevel.merchantLevel}">
						selected=selected </c:if> >${merchantLevel.merchantName}</option>
					</c:forEach>
				</select>
...
<!-- 					<a href="${ctx}/product/toShowProductIdentityPrice?id=${UpProductRelation.id}">[查看]</a> --%>
 					<a href="${ctx}/product/toEditUpProductRelation?id=${UpProductRelation.id}&merchantId=${UpProductRelation.identityId}">[修改]</a>
					<c:if test="${UpProductRelation.status=='0'}">
						<a href="${ctx}/product/updateUpProductRelationStatus?id=${UpProductRelation.id}&status=1&merchantId=${UpProductRelation.identityId}">[开启]</a>
					</c:if>
					<c:if test="${UpProductRelation.status=='1'}">
						<a href="${ctx}/product/updateUpProductRelationStatus?id=${UpProductRelation.id}&status=0&merchantId=${UpProductRelation.identityId}">[关闭]</a>
					</c:if>
					<a href="${ctx}/product/deleteUpProductRelationById?id=${UpProductRelation.id}&merchantId=${UpProductRelation.identityId}">[删除]</a>
					-->
				</td>
			</tr>
		</c:forEach></c:when>
	...
	<div class="pages_menber">(共<SPAN id=pageTotal>${counttotal}</SPAN>条记录)</div>
 	...
 	<script type="text/javascript" language="javascript">
		...
		$(document).ready(function(){
					...
					$('input[type="checkbox"]').each(function(){
						...
						if(b==0){
							...
							if($('#productStatus_'+b).val()=='0'){
								...
								// $("#merchantselect_"+b).attr("disabled",false);
									if(b==1){
									...
									$("#merchantselect_"+b).attr("disabled",true);
									$("#discountButton_"+b).hide();
									$("#discountChangesButton_"+b).hide();
								}
							}
			   ...
			   $('#changes_discount').click(function(){
				   ...
				   $(":checkbox:checked",'#tb_list').each(function(){
				     // 行下标;
				     var b=Number($("table input:checkbox").index(this));
				     var id = $("#id_"+b).val();                               // document.getElementById("listTable").rows[b].cells[1].childNodes[0].value;
				     var productId = $("#productId_"+b).val();            // document.getElementById("listTable").rows[b].cells[2].childNodes[0].value;
				     var discount =$("#discount_"+b).val();               // document.getElementById("listTable").rows[b].cells[3].childNodes[0].value;
				     var level =  $("#merchantselect_"+b).val();              // document.getElementById("listTable").rows[b].cells[7].childNodes[0].value;
				     ...
				     upProductData+=id+"_"+discount+"_"+productId+"_"+level+"__";
				   });
				   if(war==0){
					   war=1;
				   ...
				   if(upProductData!=""){
					// window.location.href="${ctx}/product/toEditSupplyProductRelations?upProductData="+upProductData;
					   $.ajax({
						   url:'toEditSupplyProductRelations',
						   type: 'GET',
                           data: "upProductData="+upProductData, 
			   ...
			   $('#changes_delete').click(function(){
				   ...
				   $(":checkbox:checked",'#tb_list').each(function(){
				    ...
				    if(deleteid!=""){
				    	$.ajax({
							   url:'deleteSupplyProductRelationList',
							   type: 'GET',
	                           data: "ids="+deleteid, 
	                           contentType:'application/text;charset=UTF-8', 
	                           success: function(result) {
		...
		function discountChanges(){
			var discount = "";
			 $(":checkbox:checked",'#tb_list').each(function(n,v){
			     ...
			     // 行下标;
			    	// var b=Number($("table input:checkbox").index(this));
			    	// var discount=$("#discount_"+obj).val();
			    	/*
					 * if(b==1){
					 * 
					 * }else{ if($('#productStatus_'+b).val()=='0'){ }else{
					 * $("#discount_"+b).val(discount); } }
					 */
			   });
		}
		...
		function getProductStatus(id,status){
			var cfg = {
    						url:'batchUpdateSupplyProductRelationStatus',
   							type: 'GET',
		...
		function back(){
			window.location.href='${ctx}/product/allSupplyProductRelation';
		}
	</script> 
</body>
</html>
