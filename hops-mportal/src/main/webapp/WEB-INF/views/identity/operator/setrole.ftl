...
function addrole(oid){
    ...
    $.ajax({
        type: "GET",
        data: null,
        url: "setRoleDo?oid="+oid+"&roleid="+result+"&t="+ new Date().getTime()+"&source=${source}",
		async: false,
		...
		<table class="input">
			...
			<tr>
				...
				<td>
					...
					<input type="button" class="button" value="返&nbsp;&nbsp;回" onclick="window.location.href='${base}${backUrl}'" />
				</td>
			</tr>
		</table>
	</form>
