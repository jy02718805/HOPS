...
<script>
	function subCreditableBanlance(){
		...
		if (confirm("确认给[${identityName}]授信减款 [" + amount+"]元")) {
			$('#inputForm').submit();
		}
	}
	function check(v){
...
<body>
<div class="mg10"></div>
	<form id="inputForm" action="${ctx}/account/doSubCreditableBanlance"  method="post">
		...
		<input 	type="hidden" id="accountType.accountTypeId" name="accountType.accountTypeId" value="${accountTypeId}" /> 
		<input type="hidden" id="pageType" name="pageType" value="${pageType}"/>
		<input type="hidden" id="creditableBanlance" name="creditableBanlance"/>
		<table class="input">
			...
			<tr>
				<th>
					<span class="requiredField">*</span>授信余额减款金额(元):
				</th>
				<td>
			...
			<tr>
				<th>
					<span class="requiredField">*</span>授信余额减款原因:
				</th>
				<td>
			...
			<tr>
				...
				<td>
					<input type="button" class="button" value="确&nbsp;&nbsp;定" onclick="subCreditableBanlance()"/>
