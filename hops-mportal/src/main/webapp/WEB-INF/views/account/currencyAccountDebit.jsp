...
<script>
	function doCurrencyAccountDebit() {
		...
		if (confirm("确认给[${identityName}]加款 [" + amount+"]元")) {
			$('#inputForm').submit();
		}
...
<body>
	...
	<div class="mg10"></div>
	<form id="inputForm" action="${ctx}/account/doCurrencyAccountDebit"
		method="post">
		<input type="hidden" id="accountId" name="accountId"
			value="${currencyAccount.accountId}" /> <input type="hidden"
			id="externalAccountId" name="externalAccountId" /> <input
			type="hidden" id="identityId" name="identityId" value="${identityId}" />
		<input type="hidden" id="accountTypeId" name="accountTypeId"
			value="${accountTypeId}" /> <input type="hidden" id="pageType"
			name="pageType" value="${pageType}" /> <input type="hidden"
			id="availableBalance" name="availableBalance" />
		<table class="input">
			...
			<tr>
				<th><span class="requiredField">*</span>外部银行账户:</th>
				<td><select name="externalAccounts" id="externalAccounts"
					class="select w150">
			...
			<tr>
				<th><span class="requiredField">*</span>可用余额加款金额(元):</th>
			...
			<tr>
				<th><span class="requiredField">*</span>可用余额加款原因:</th>
			...
			<tr>
				...
				<td><input type="button" class="button" value="确&nbsp;&nbsp;定"
					onclick="doCurrencyAccountDebit()" /> <input type="button"
