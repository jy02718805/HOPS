...
<body>
	<form id='listForm' method='post' action="${base}/report/transactionReportsByNum">
		...
		<div class="line_bar">
			...
			报表类型:
			<select  id='reportType' name='reportType' onchange="submitReportType();" class="select w100">
			...
			<div style="float:right;">
				 <@shiro.user>
				<input type="button" class="button" onclick="queryTransactionReportsByNum();"  value="查询"/>
				</@shiro.user>
			...
			<#list reportTermList as reprtterm>
			<#if reprtterm.reportMetadata.metadataField=="ORDER_NO">
			订单号:<input type="text" id='orderNo' name='orderNo'  value="${transactionReportVo.orderNo!""}" class="ipt w80"/>
			</#if>
			</#list>
			...
			商户名称: 
			<input type="text" class="easyui-combobox" id='merchantName' name='merchantName'  value="${transactionReportVo.merchantName!''}" class="ipt w150" data-options="cache:false,valueField:'id',textField:'text'"/>
			商户类型:
			<select  id='merchantType' name='merchantType' class="select w80">
			<option value="">请选择</option>
				<option value="SUPPLY"  <#if (transactionReportVo.merchantType!)=="SUPPLY">selected='selected'</#if>>供货商</option>
				<option value="AGENT"  <#if (transactionReportVo.merchantType!)=="AGENT">selected='selected'</#if>>代理商</option>
			</select>
			</#if>
			</#list>
			<#list reportTermList as reprtterm>
			...
			状态:
				<select name="reportsStatus" id="reportsStatus" class='select w80'>
					<option value="">请选择</option>
	 ...
	 <script type="text/javascript" language="javascript"> 
		...
		PageClick = function(pageclickednumber) { 
			...
			$("#page").val(pageclickednumber);
		   	$("#listForm").attr("action","${base}/report/transactionReportsByNum").submit();
		
		}
		
		
		function queryTransactionReportsByNum(){
			...
			var s = $('#merchantName').combobox('getText');
			$('#merchantName').combobox('setValue',s);
			$("#listForm").attr("action","${base}/report/transactionReportsByNum").submit();
		}
		function getUsers(){
			
			var url='${base}/report/getUsers?identityType='+'MERCHANT';
			
			$('#merchantName').combobox('reload', url);
		...
		function exportTransactionExport(){
			...
			var s = $('#merchantName').combobox('getText');
			$('#merchantName').combobox('setValue',s);
			$("#listForm").attr("action","${base}/report/transactionExportByNum").submit();
		}
