...
<body>
	<form id='listForm' method='post' action="${base}/report/profitReports">
	<input type="hidden"  id='reportTypeName' name='reportTypeName'  value="everyDayProfit" />
		<input id='page' type=hidden name='page'> 
		<div class="line_bar"> 
			开始时间:<input id="beginTime" name="beginTime" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" value="${profitReportVo.beginTime!""}" class='ipt w80'/>
			结束时间:<input id="endTime" name="endTime" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" value="${profitReportVo.endTime!""}" class='ipt w80'/>
			报表类型:<select  id='reportType' name='reportType' onchange="submitReportType();" class="select w80">
			<option value="">请选择</option>
				<#list reportTypeList as rt>
					<option value="${rt.reportTypeId!""}"  <#if rt.reportTypeId+""==(profitReportVo.reportType!)>selected='selected'</#if>>${rt.reportFileName}</option>
				</#list>
			</select>
			...
			<#if reprtterm.reportMetadata.metadataField=="PROVINCE_NAME">
			 省份:<select name="province" id="province" onchange="getCityByProvince(this)" class="select w80">
						...
						<#list provinces as pn>
								<option value="${pn.provinceId}"  <#if pn.provinceId==(profitReportVo.province!)>selected='selected'</#if>>${pn.provinceName}</option>
						</#list>
					 </select>
			</#if>
			</#list>
			...
			<select name="city" id="city" style="width:80px" class="select w80">
						<option value="">请选择</option>
						<#list citys as ct>
								<option value="${ct.cityId!""}" <#if (ct.cityId==(profitReportVo.city!))>selected='selected'</#if>>${ct.cityName!""}</option>
						</#list>
					 </select>
			</#if>
			</#list>
			...
			<select name="carrierNo" id="carrierNo" class="select w80">
						...
						<#list carrierInfos as cafo>
								<option value="${cafo.carrierNo}" <#if (cafo.carrierNo==(profitReportVo.carrierNo!))>selected='selected'</#if>>${cafo.carrierName}</option>
						</#list>
					 </select>
			</#if>
			</#list>
			...
			面值:<input type="text" id='parValue' name='parValue'  value="${profitReportVo.parValue!""}" class="ipt w80" />
			</#if>
			</#list>
			...
			业务类型:
				<select name="businessType" id="businessType" class='select w80'>
					<option value="">请选择</option>
					<option value="0" <#if (profitReportVo.businessType!)=='0'>selected="selected"</#if>>话费</option>
					<option value="1" <#if (profitReportVo.businessType!)=='1'>selected="selected"</#if>>流量</option>
				</select>
			</#if>
			</#list>
			...
			商户名称:<input type="text"  id='merchantName' name='merchantName'  value="${profitReportVo.merchantName!""}" class="ipt w150"/>
			商户类型:
			<select  id='merchantType' name='merchantType' class="select w80">
			<option value="">请选择</option>
				<option value="SUPPLY"  <#if (profitReportVo.merchantType!)=="SUPPLY">selected='selected'</#if>>供货商</option>
				<option value="AGENT"  <#if (profitReportVo.merchantType!)=="AGENT">selected='selected'</#if>>代理商</option>
			</select>
			</#if>
				</#list>
				</div>
	<table  id='listtable' class='list'>
	<#if (profitReportVo.reportType!)=="">
	<thead>
		<tr>
			<th>统计时间</th>
			<th>商户名称</th>
	...
	${profits!""}
	 ...
	 <script type="text/javascript" language="javascript"> 
		...
		PageClick = function(pageclickednumber) { 
		   ...
		   $("#listForm").attr("action","${base}/report/profitReports").submit();
		}
		...
		function exportProfitReport(){
			...
			$("#listForm").attr("action","${base}/report/profitExport").submit();
		}
