...
public class CookieUtils
{
                                 ...
                                 int maxAge)
    {
        ...
        if (StringUtil.isNotBlank(SpringUtils.getApplicationContext().getApplicationName()))
        {
        ...
        {
        	logger.error("CookieUtils:[setCookie-UnsupportedEncodingException]["+e.getMessage()+"]");
        }
        ...
        if (cookies != null)
        {
            for (Cookie cookie : cookies)
            {
                if (cookie.getName().equals(name))
                {
                    ...
                    {
                    	logger.error("CookieUtils:[getCookie-UnsupportedEncodingException]["+e.getMessage()+"]");
                    }
                    if (isRemove)
                    {
                        cookie.setMaxAge(0);
