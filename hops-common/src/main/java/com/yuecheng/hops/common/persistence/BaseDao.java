...
//	        page.setCount(Long.valueOf(createQuery(countQlString, parameter).uniqueResult().toString()));
	...
	public void save(T entity){
		try {
			...
			logger.error("BaseDao:[save-IllegalArgumentException]["+e.getMessage()+"]");
		} catch (IllegalAccessException e) {
			logger.error("BaseDao:[save-IllegalAccessException]["+e.getMessage()+"]");
		} catch (InvocationTargetException e) {
			logger.error("BaseDao:[save-InvocationTargetException]["+e.getMessage()+"]");
		}
...
//	        page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
	...
	public Page<T> find(Page<T> page, DetachedCriteria detachedCriteria, ResultTransformer resultTransformer) {
		...
		// order by
		if (StringUtil.isNotBlank(page.getOrderBy())){
			for (String order : StringUtil.split(page.getOrderBy(), ",")){
	...
	public long count(DetachedCriteria detachedCriteria) {
		...
		try {
			...
			logger.error("BaseDao:[count-NoSuchFieldException]["+e.getMessage()+"]");
		} catch (IllegalAccessException e) {
			logger.error("BaseDao:[count-IllegalAccessException]["+e.getMessage()+"]");
		}
		return totalCount;
	}
