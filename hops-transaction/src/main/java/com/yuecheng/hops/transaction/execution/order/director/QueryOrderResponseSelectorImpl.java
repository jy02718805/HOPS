...
@Service("queryOrderResponseSelector")
public class QueryOrderResponseSelectorImpl implements QueryOrderResponseSelector
{
    private static final Logger logger = LoggerFactory.getLogger(QueryOrderResponseSelectorImpl.class);
    ...
    public QueryDeliveryAction select(Delivery delivery,Map<String, Object> responseMap)
    {
        ...
        try
        {
            ...
            if (null != merchantStatus)
            {
            	merchantStatus = merchantStatus.trim();
            }
            String deliveryResult = (String)responseMap.get(EntityConstant.MerchantResponse.DELIVERY_RESULT);
            
            String orderSuccessFeeStr = (String)responseMap.get(EntityConstant.MerchantResponse.ORDER_SUCCESS_FEE);
            String interfaceType = (String)responseMap.get(TransactionMapKey.INTERFACE_TYPE);
            BigDecimal orderSuccessFee=null;
            //!!!分try模块
            try
            {
                orderSuccessFee = StringUtils.isNotBlank(orderSuccessFeeStr)?new BigDecimal(orderSuccessFeeStr):delivery.getProductFace();
            }
            catch (Exception e)
            {
