...
@Scope("prototype")
@Service("deliveryStatusSendingAction")
public class DeliveryStatusSendingAction extends ActionHandler
{
    private static Logger           logger = LoggerFactory.getLogger(DeliveryStatusSendingAction.class);
        ...
        throws HopsException
    {
        ...
        try
        {
            ...
            logger.debug("修改发货记录状态(DELIVERY_STATUS_WAIT->DELIVERY_STATUS_SENDING) [开始]"+delivery.getDeliveryId());
            delivery = (Delivery)deliveryStatusManagementServiceImpl.updateStatus(delivery.getDeliveryId(),
                delivery.getDeliveryStatus(), Constant.Delivery.DELIVERY_STATUS_SENDING);
           // deliveryManagement.updateDeliveryStatus(delivery.getDeliveryStatus(), deliveryStatus, delivery.getDeliveryId());
            ...
            logger.debug("修改发货记录状态(DELIVERY_STATUS_WAIT->DELIVERY_STATUS_SENDING) [结束]"+delivery.getDeliveryId());
            
            ActionContextUtil.setActionContext(ActionMapKey.DELIVERY, delivery);
        }
        catch (Exception e)
        {
            logger.error("deliveryStatusSendingAction happen Exception caused by "+ ExceptionUtil.getStackTraceAsString(e));
            throw new ApplicationException("transaction002006", new String[]{ExceptionUtil.getStackTraceAsString(e)}, e);
        }
    }
}
