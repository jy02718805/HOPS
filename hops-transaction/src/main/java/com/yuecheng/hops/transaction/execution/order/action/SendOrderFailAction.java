...
@Service("sendOrderFailAction")
public class SendOrderFailAction implements SendOrderAction
{
    private static final Logger          logger = LoggerFactory.getLogger(SendOrderFailAction.class);
    ...
    {
        logger.debug("订单发送流程:接收状态转义[Fail]!"+String.valueOf(delivery).toString());
        ...
        {
            logger.error("sendOrderFailAction happen Exception caused by "+ ExceptionUtil.getStackTraceAsString(e) + "  params[" + String.valueOf(frozenAmt).toString() + "  " + String.valueOf(payee).toString() + "  " + String.valueOf(order).toString()+"]");
            throw new ApplicationException("transaction002023", e);
        }
    }
