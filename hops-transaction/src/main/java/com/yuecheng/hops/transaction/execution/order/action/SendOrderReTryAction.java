...
@Service("sendOrderReTryAction")
public class SendOrderReTryAction implements SendOrderAction
{
    private static final Logger          logger = LoggerFactory.getLogger(SendOrderReTryAction.class);
    ...
    {
        logger.debug("进入重试流程"+String.valueOf(delivery).toString());
        ...
        {
            logger.error("sendOrderReTryAction happen Exception caused by "+ ExceptionUtil.getStackTraceAsString(e) + "  params[" + String.valueOf(frozenAmt).toString() + "  " + String.valueOf(payee).toString() + "  " + String.valueOf(order).toString()+"]");
            throw new ApplicationException("transaction002025", e);
        }
    }
